...
const page = () => {
  ...
  // ModalDelete
  const { delete_Category } = ApiActions()
  ...
  const deleteEventHandler = async () => {
    await delete_Category(idDelete).then((res => {
      console.log(res)
      if (res) {
        toggleRerender()
      }
  ...
  useEffect(() => {
    ...
    getApi(`/api/category?${(new URLSearchParams(data)).toString()}`, setData, setLoading)
      ...
      >
        ...
        >
          ...
          {loading ? LoadingState() :
            data?.data?.length === 0 ? renderEmptyState() : renderCategories()
          }
