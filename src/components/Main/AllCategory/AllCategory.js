          ...
          <div className="max-w-[1500px] w-full grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-2 justify-center ">
            ...
            {data &&
              ...
              data.data.map((e, i) => {
                return (
                  <Link
                    href={`/product/${e.route}`}
                    key={`${i}354`}
                    className=" flex flex-col text-gray-700 bg-white shadow-lg border-[3px] border-gray-200 bg-clip-border rounded-xl w-full h-min hover:border-[#f548485d] transition-all duration-500 ease-in-out"
                  >
                    <div className="mx-2 mt-2 sm:mx-3 md:mx-4 sm:mt-3 md:mt-4 overflow-hidden text-gray-700 bg-white shadow-lg bg-clip-border rounded-lg">
                      <Image
                        ...
                        src={`data:image/webp;base64,${e.imageBase64}`}
                        alt="profile-picture"
                      />
                    </div>
                    <div className="p-4 sm:p-6 text-center">
                      ...
                      <p className="text-right block mb-2 vazirDemibold max-sm:h-[35px] text-[13px] sm:text-md md:text-lg antialiased leading-snug tracking-normal text-blue-gray-900 ellipsisOneLine md:!hidden max-md:block">
                        {e.title}
                      </p>
                      <p className="text-right block vazirLight max-sm:h-[30px] text-[11px] sm:text-sm antialiased leading-snug tracking-normal text-blue-gray-900 text-gray-500 ellipsisOneLine md:!hidden max-md:block">
                        {e.description}
                      </p>
                    </div>
                  </Link>
